...
"""
do_1d_met_toys.py
Generates and fits 1D MET-only toy datasets from the signal + non-peaking-background
model, reusing the same fitted shapes as do_toys.py / toy_noPkgBkg.py's
get_signal_model / get_background_model, but keeping only the MET marginal pdfs
(pdf_of_spline_<m1>_<m2>, bkgnonpeak_met) instead of the full (m(ll), MET) product
pdfs, and fitting with a bare RooMCStudy (like toy_noPkgBkg.py's own __main__ block)
rather than through real Combine commands -- this is a quick closure/recovery check,
not a limit-setting study, so there is no rMin/rMax POI machinery and no
AsymptoticLimits step. Sibling of do_1d_mll_toys.py, same structure, MET instead of
m(ll).

Model: sig_pdf = MET spline (fitted signal shape at one mass point)
       bkg_pdf = MET Gumbel (non-peaking-only background shape)
...
Usage:
    python3 do_1d_met_toys.py
    python3 do_1d_met_toys.py -N 500 --n-sig-in 0 --n-bkg-in 48
    python3 do_1d_met_toys.py --m1 650 --m2 1 --seed 42
"""
...
def _find_component(components, name):
    """Exact-name lookup (see _find_component_prefix for pdf_of_spline_<m1>_<m2>,
    whose name carries the mass point)."""
    for c in components:
...
def _find_component_prefix(components, prefix):
    """Prefix lookup -- get_signal_model's pdf_of_spline is fetched from the workspace
    as pdf_of_spline_<m1>_<m2>, so its name is mass-point-dependent."""
    for c in components:
        if hasattr(c, "GetName") and c.GetName().startswith(prefix):
            return c
    raise RuntimeError(f"no component starting with '{prefix}' found among {[c.GetName() for c in components if hasattr(c, 'GetName')]}")


def build_model_1d(m1, m2):
    """
    Build (met, sig_pdf_met, bkg_pdf_met, components) for one mass point: the MET-only
    spline signal shape and Gumbel non-peaking-background shape, reusing
    toy_noPkgBkg.py's fitted shapes (same setup/chdir pattern as do_toys.py's
    build_model -- get_signal_model/get_background_model read paths relative to
    hbb_zll/toy_experiments/ and a bare module-level `mll` global, needed internally
    to build the m(ll) marginal pdfs even though those are discarded here).
    ...
    sig_pdf_met = _find_component_prefix(sig_components, "pdf_of_spline")
    bkg_pdf_met = _find_component(bkg_components, "bkgnonpeak_met")
    return met, sig_pdf_met, bkg_pdf_met, sig_components + bkg_components


# ── Toy generation + fitting ─────────────────────────────────────────────────────────

def run_mcstudy(met, sig_pdf, bkg_pdf, n_sig_in, n_bkg_in, n_sig_range, n_bkg_range,
                n_experiments, seed):
    n_sig = ROOT.RooRealVar("n_sig", "n_sig", n_sig_in, *n_sig_range)
    n_bkg = ROOT.RooRealVar("n_bkg", "n_bkg", n_bkg_in, *n_bkg_range)
    total_pdf = ROOT.RooAddPdf("total_pdf_1d_met", "total_pdf_1d_met",
...
def make_example_toy_plot(mcs, met, total_pdf, sig_pdf, bkg_pdf, n_sig, n_bkg,
    ...
    frame = met.frame(ROOT.RooFit.Title(f"Toy #{toy_index}: data + best fit"))
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate and fit 1D MET-only toy datasets from the signal + "
    ...
    parser.add_argument("--outdir", default="/eos/cms/store/group/phys_susy/skkwan/toys/1d_met")
    ...
    for example_idx in _first_converged_toys(rows, 5):
        make_example_toy_plot(mcs, met, total_pdf, sig_pdf, bkg_pdf, n_sig, n_bkg,
