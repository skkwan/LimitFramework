#!/usr/bin/env python3
"""
plot_boundary_toys_met.py
Reads a results_*.root file produced by do_1d_met_toys.py (fitResult_i / genData_i
...
to an interior minimum. MET sibling of plot_boundary_toys.py.

Usage:
    python3 plot_boundary_toys_met.py --results /path/to/results_..._N5000_seed1.root \
        --m1 650 --m2 1 -n 3
"""
...
from do_1d_met_toys import build_model_1d


def find_boundary_toys(results_path, n_max):
    """Return (TFile, indices of first n_max toys with n_sig pegged at the max n_sig
...
def plot_toy(f, met, total_pdf, sig_pdf, bkg_pdf, n_sig, n_bkg, plots_dir, tag, toy_index):
    ...
    frame = met.frame(ROOT.RooFit.Title(f"Toy #{toy_index}: data + best fit (n_sig at boundary)"))
...
def main():
    ...
    parser.add_argument("--results", required=True, help="Path to results_*.root from do_1d_met_toys.py")
    ...
    total_pdf = ROOT.RooAddPdf("total_pdf_1d_met_boundary", "total_pdf_1d_met_boundary",
                               ROOT.RooArgList(sig_pdf, bkg_pdf),
                               ROOT.RooArgList(n_sig, n_bkg))

    for idx in boundary_idx:
        plot_toy(f, met, total_pdf, sig_pdf, bkg_pdf, n_sig, n_bkg, plots_dir, tag, idx)
